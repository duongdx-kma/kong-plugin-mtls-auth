...
local function extract_and_add_rdn(t, dn, startPos, endPos)
    local delimiterPos = string.find(dn, "=", startPos)
...
local function parse_dn(dn)
    ...
    local nextRdn = 1
    while nextRdn <= #dn do
        local endOfRdnPos = string.find(dn, "[^\\],", nextRdn)
        if not endOfRdnPos then
            endOfRdnPos = #dn
        end
...
function MtlsAuth:access(config)
    -- Check TLS client verification status
    ...
    local cert_dn = parse_dn(ngx.var.ssl_client_s_dn or "")
